...
--ADVANCED JOIN:
	...
	--4.INTERVIEWS
		SELECT C.CONTEST_ID, CT.HACKER_ID, CT.NAME, T.SUM_SUBMI, T.SUM_ACCEPT, T.SUM_VIEWS, T.SUM_UNIQUE 
		FROM (SELECT COLLEGE_ID, SUM(SUM_VIEWS) AS SUM_VIEWS, SUM(SUM_UNIQUE) AS SUM_UNIQUE, SUM(SUM_SUBMI) AS SUM_SUBMI, 
		SUM(SUM_ACCEPT) AS SUM_ACCEPT 
		FROM 
		(SELECT C.COLLEGE_ID, V.CHALLENGE_ID, V.SUM_VIEWS, V.SUM_UNIQUE, V.SUM_SUBMI, V.SUM_ACCEPT 
		FROM CHALLENGES C 
		RIGHT OUTER JOIN  
		(SELECT V.CHALLENGE_ID, V.SUM_VIEWS, V.SUM_UNIQUE, S.SUM_SUBMI, S.SUM_ACCEPT 
		FROM 
		(SELECT CHALLENGE_ID, SUM(TOTAL_VIEWS) AS SUM_VIEWS, SUM(TOTAL_UNIQUE_VIEWS) 
		AS SUM_UNIQUE FROM VIEW_STATS GROUP BY CHALLENGE_ID) V 
		LEFT OUTER JOIN 
		(SELECT CHALLENGE_ID, SUM(TOTAL_SUBMISSIONS) 
		AS SUM_SUBMI, SUM(TOTAL_ACCEPTED_SUBMISSIONS) AS SUM_ACCEPT FROM SUBMISSION_STATS GROUP BY CHALLENGE_ID) S 
		ON V.CHALLENGE_ID=S.CHALLENGE_ID) V
		ON V.CHALLENGE_ID=C.CHALLENGE_ID) GROUPING 
		GROUP BY COLLEGE_ID) T 
		INNER JOIN COLLEGES C ON C.COLLEGE_ID=T.COLLEGE_ID 
		INNER JOIN CONTESTS CT ON CT.CONTEST_ID=C.CONTEST_ID
		WHERE T.SUM_SUBMI<>0 OR T.SUM_ACCEPT <>0 OR T.SUM_VIEWS!=0 OR  T.SUM_UNIQUE!=0
